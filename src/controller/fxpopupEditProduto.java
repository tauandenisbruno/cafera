...
* 
*  Responsável pelo popup com os campos para o usuário editar um produto existente no estoque
*/
...
public class fxpopupEditProduto
 {
     ...
     private void initProduto()
     {
        ...
        prod_nome = produtoSelecionado.getProdutoNome();
        prod_preco = produtoSelecionado.getProdutoPreco();
        prod_estoque = produtoSelecionado.getProdutoEstoque();

        txtfIdProduto.setText("" + prod_id);
        txtfNomeProduto.setText(prod_nome);
        txtfPrecoProduto.setText("" + prod_preco);
        txtfEstoqueProduto.setText("" + prod_estoque);
    ...
    private void setChoiceBoxCategoria()
    {
        ...
        {
            produto produtoSelecionado = editProd.getProdutoSelecionado();
            String categoria = produtoSelecionado.getProdutoCategoria();
            choiceCategoria.setValue(categoria);
        }
    }
    // -----------------------------------------------------
    ...
    private void setChoiceBoxFornecedor()
    {
        ...
        {
            produto produtoSelecionado = editProd.getProdutoSelecionado();
            String fornecedor = produtoSelecionado.getProdutoFornecedor();
            choiceFonecedor.setValue(fornecedor);
        }
    }
    // -----------------------------------------------------
    ...
    void actionConfirmar(ActionEvent event) throws IOException
    {
        try
        {
            ...
            if(prod_nome != null && !prod_nome.trim().isEmpty())
            {
                ...
                sqlite.editarProduto(prod_id, prod_nome, prod_preco, prod_estoque, choiceCategoria.getValue(), choiceFonecedor.getValue());
